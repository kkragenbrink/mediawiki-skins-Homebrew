<?php
/**
 * Homebrew - Modern version of MonoBook with fresh look and many usability
 * improvements.
 *
...
/**
 * QuickTemplate class for Homebrew skin
 * @ingroup Skins
 */
class HomebrewTemplate extends BaseTemplate {
	/* Functions */
	...
	public function execute() {
		...
		if ( $this->config->get( 'HomebrewUseIconWatch' ) ) {
...
<?php
    ...
    public function html ($str) {
        if (strlen($this->data[$str]) === 0) { return; }

        if ($str === 'bodycontent') {
            $contents = preg_split( "/<p>\+\+\+\+\+.*?<\/p>/ms", $this->data[$str]);

            echo '<div id="bodyContent" class="mw-body-content">';
            echo '<div class="mw-body-content-wrapper" id="page-1">';
            foreach ($contents as $no => $page) {
                echo $page;
                echo sprintf('<div class="page-number">%s</div>', $no+1);
                echo '</div>';
                if ($no+1 !== count($contents)) {
                    echo '</div>';
                    echo sprintf('<div class="mw-body-content-wrapper" id="page-%s">', $no+2);
                    echo sprintf('<div id="mw-content-text-%s" lang="en" dir="ltr" class="mw-content-ltr">', $no+1);
                }
            }
            echo '</div>';
        }
	...
	protected function renderNavigation( $elements ) {
		...
		foreach ( $elements as $name => $element ) {
			switch ( $element ) {
				case 'NAMESPACES':
					?>
					<div id="p-namespaces" role="navigation" class="homebrewTabs<?php
				...
				case 'VARIANTS':
					?>
					<div id="p-variants" role="navigation" class="homebrewMenu<?php
				...
				case 'VIEWS':
					?>
					<div id="p-views" role="navigation" class="homebrewTabs<?php
				...
				case 'ACTIONS':
					?>
					<div id="p-cactions" role="navigation" class="homebrewMenu<?php
					if ( count( $this->data['action_urls'] ) == 0 ) {
						echo ' emptyPortlet';
					}
					?>" aria-labelledby="p-cactions-label">
						<h3 id="p-cactions-label"><span><?php
							$this->msg( 'homebrew-more-actions' )
						?></span><a href="#"></a></h3>
				...
				case 'SEARCH':
					...
					<div id="p-search" role="search">
						...
						<form action="<?php $this->text( 'wgScript' ) ?>" id="searchform">
							<div<?php echo $this->config->get( 'HomebrewUseSimpleSearch' ) ? ' id="simpleSearch"' : '' ?>>
							<?php
